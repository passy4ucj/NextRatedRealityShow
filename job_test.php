...
if (isset($_POST["job_allocate"])) {
  ...
  for ($count=0; $count <= $_POST["total_item"]; $count++) { 
    ...
    $statement = $connect->prepare("
      INSERT INTO job_allocation (staff_id, order_id, order_reciever_name, order_date, item_name, order_item_quantity job_type, job_status)
      VALUES (:staff_id, :order_id, :order_reciever_name, :order_date, :item_name, :order_item_quantity, :job_type, :job_status)
      ");
    $statement->execute(
      array(':staff_id' => $staff_id, ':order_id' => $order_id, ':order_reciever_name' => trim($_POST["order_reciever_name"]), ':order_date' => trim($_POST["order_date"]), ':item_name' => trim($_POST["item_name"][$count]), ':order_item_quantity' => trim($_POST["order_item_quantity"][$count]), ':job_type' => trim($_POST["job_type"][$count]), ':job_status' => $job_status  )
      );
  }
  ...
  if (isset($_GET["allocate"]) && isset($_GET["id"])) {
        ...
        <td colspan="2">
          ...
          <tr>
            ...
            <td><input type="text" name="job_type[]" id="job_type<?php echo $m; ?>"  class="form-control input-sm " /></td>
            <?php
            $work_status = "YES";
            $statement = $connect->prepare("
